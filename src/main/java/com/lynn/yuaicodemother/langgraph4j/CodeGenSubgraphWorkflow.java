package com.lynn.yuaicodemother.langgraph4j;
...
import com.lynn.yuaicodemother.langgraph4j.model.QualityResult;
import com.lynn.yuaicodemother.langgraph4j.node.*;
import com.lynn.yuaicodemother.langgraph4j.node.concurrent.*;
...
public class CodeGenSubgraphWorkflow {
    ...
    public WorkflowContext executeWorkflow(String originalPrompt) {
        ...
        int stepCounter = 1;
        // 配置并发执行：用于执行 image_plan 后的四个子图（并发分支）
        ExecutorService pool = ExecutorBuilder.create()
                .setCorePoolSize(10)
                .setMaxPoolSize(20)
                .setWorkQueue(new LinkedBlockingQueue<>(100))
                .setThreadFactory(ThreadFactoryBuilder.create().setNamePrefix("Parallel-Subgraph").build())
                .build();
        RunnableConfig runnableConfig = RunnableConfig.builder()
                // 为 image_plan 节点配置线程池，使其后的并发分支能利用该池
