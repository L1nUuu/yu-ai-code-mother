    ...
    public Flux<String> generatorAndSaveCodeStream(String userMessage, CodeGenTypeEnum codeGenTypeEnum){
        ...
        return switch (codeGenTypeEnum){
            case HTML:
                ...
                yield processCodeStream(htmlCodeStream,CodeGenTypeEnum.HTML); //处理结果流（解析保存）
            case MULTI_FILE:
                Flux<String> multiFileCodeStream = aiCodeGeneratorService.generateMultiFileCodeStream(userMessage);//生成结果流
                yield processCodeStream(multiFileCodeStream,CodeGenTypeEnum.MULTI_FILE);//处理结果流（解析保存）
            default:
                String errorMessage = "不支持的生成类型" + codeGenTypeEnum.getValue();
                throw new BusinessException(ErrorCode.SYSTEM_ERROR,errorMessage);
        };
        ...
        return codeStream
                ...
                .doOnComplete(() -> {
                    try {
                        ...
                        //使用代码保存执行器保存代码
                        File saveFir = CodeFileSaverExecutor.executeSaveCode(parsedResult,codeGenTypeEnum);
